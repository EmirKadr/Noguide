...
def search_documents(query, visible_count=5):
    ...
    for doc, score, _ in results:
        ...
        # OBS: preview ligger i <summary> → syns när stängd och även när öppen (för Dokument)
        html_output += f"""
        <details class='doc-result'>
          <summary>
            <div class='preview'>{snippet}</div>
            <span class='toggle'>Läs mer</span>
          </summary>
          <div class='full'>
            <p>🔍 <b>Matchningspoäng:</b> {round(score, 1)}</p>
            <p>{download_link}</p>
          </div>
        </details>
        <hr>
...
def search_word_doc(query, visible_count=5):
    ...
    for section, score in results:
        ...
        # För Allmänt ska preview försvinna när öppet → därför class='result' och preview i <summary>
        html_output += f"""
...
def render_markdown_section_to_html(section_text):
    ...
    def flush_code():
        ...
        code_str = "\n".join(code_buf)
        # välj lexer: från språk eller enkel heuristik
        try:
            if code_lang:
...
def search_developer_doc(query, visible_count=5):
    ...
    for sec, score in results:
        ...
        # Developer: samma beteende som Allmänt → preview i <summary>, döljs när open
        html_output += f"""
        <details class='result'>
          <summary>
            <div class='preview'>{snippet}</div>
...
# =========================
custom_css = """
/* Gemensam look för preview-rutor */
details.result > summary .preview,
details.doc-result > summary .preview {
  display: block;
  background:#f6f6f6;
  padding:10px;
  border-radius:5px;
...
/* Dokument – preview ska alltid vara synlig (ingen hide-regel för .doc-result) */

/* Ikoner och klickbarhet */
details.result > summary,
details.doc-result > summary {
  cursor: pointer;
  list-style: '▶️ ';
}
details.result[open] > summary,
details.doc-result[open] > summary {
  list-style: '▼ ';
}
