...
if os.path.exists(word_doc_path):
    try:
        ...
        print("⚠️ Kunde inte läsa Bibliotek.docx (öppen/låst?):", e)
...
# =======================================================
#        Markdown-stöd för quickSearch/Developer.md
# =======================================================
...
def load_developer_sections_fresh():
    """Läser Developer.md vid varje sökning så du slipper starta om appen efter ändringar."""
    if not os.path.exists(developer_md_path):
        return []
    try:
        with open(developer_md_path, "r", encoding="utf-8") as f:
            md_content = f.read()
        return parse_markdown_sections(md_content)
    except Exception as e:
        print("⚠️ Kunde inte läsa Developer.md:", e)
        return []
...
def search_developer_doc(query, visible_count=5):
    ...
    developer_sections = load_developer_sections_fresh()
...
with gr.Blocks() as demo:
    ...
    # Flik 1 – Dokument (PDF/DOCX i 'docs')
    ...
    # Flik 2 – Allmänt (Bibliotek.docx)
    with gr.Tab("Allmänt"):
        query2 = gr.Textbox(label="🔎 Sök i Bibliotek", placeholder="Ex: lager, capabilities, brandfarligt")
        output2 = gr.HTML()
        query2.change(fn=search_word_doc, inputs=query2, outputs=output2)

    # Flik 3 – Developer (Developer.md)
